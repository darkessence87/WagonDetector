...
function WDMF:Tracker_OnEvent(...)
    ...
    if event == "SPELL_CAST_START" then
        if src then
            if src.casts[spell_id] then
                local i = src.casts[spell_id].count + 1
                src.casts[spell_id].count = i
                src.casts[spell_id][i] = {}
                src.casts[spell_id][i].status = "STARTED"
            else
                src.casts[spell_id] = {}
                src.casts[spell_id].count = 1
                src.casts[spell_id][1] = {}
                src.casts[spell_id][1].status = "STARTED"
            end
    ...
    if event == "SPELL_CAST_SUCCESS" then
        if src then
            if src.casts[spell_id] then
                ...
                print('instant cast:'..spell_id.." by "..src.name.." "..src.guid)
            end
    ...
    if event == "SPELL_INTERRUPT" then
        ...
        print(event..' : '..getSpellLinkByIdWithTexture(target_spell_id)..' target:'..dst.name)
        if dst then
