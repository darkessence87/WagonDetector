...
local WDSimpleRulesModule = {}
WDSimpleRulesModule.__index = WDSimpleRulesModule

setmetatable(WDSimpleRulesModule, {
    __index = WD.Module,
    __call = function (v, ...)
...
local function updateTierRuleMenus()
    ...
    WdLib.gui:updateDropDownMenu(r, "Select encounter", WD:CreateTierList(r.callback, isEncounterActiveRuled))
    WdLib.gui:resetDropDownMenu(r, "Select encounter")

    r = WDRM.menus["export_encounter"].menus["encounters"]
    WdLib.gui:updateDropDownMenu(r, "Select encounter", WD:CreateTierList(r.callback, isEncounterRuled))
    WdLib.gui:resetDropDownMenu(r, "Select encounter")
...
local function editRuleLine(rule)
    ...
    local frame = WdLib.gui:findDropDownFrameByName(r.menus["encounters"], encounterName)
    if frame then
    ...
    -- role
    local roleFrame = WdLib.gui:findDropDownFrameByName(r.menus["roles"], rule.role)
    if roleFrame then
    ...
    -- rule
    local ruleFrame = WdLib.gui:findDropDownFrameByName(r.menus["rule_types"], rule.type)
    if ruleFrame then
    ...
    if rule.type == "EV_AURA" then
        WdLib.gui:showHiddenEditBox(r, "arg0_edit", rule.arg0)
        arg0_edit.label:SetText("Aura id:")

        WdLib.gui:updateDropDownMenu(arg1_drop, "Select action:", {{name = "apply"},{name = "remove"}})
        local frame = WdLib.gui:findDropDownFrameByName(arg1_drop, rule.arg1)
        if frame then
    ...
    elseif rule.type == "EV_AURA_STACKS" then
        WdLib.gui:showHiddenEditBox(r, "arg0_edit", rule.arg0)
        arg0_edit.label:SetText("Aura id:")
        WdLib.gui:showHiddenEditBox(r, "arg1_edit", rule.arg1)
    ...
    WdLib.gui:showHiddenEditBox(r, "arg2_edit", rule.points)
...
local function importRule(str)
    local d = WdLib.gen:decode64(str)
...
local function updateRuleLines()
    ...
    local function createFn(parent, row, index)
        ...
        if index == 1 then
            ...
            return f
        elseif index == 2 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "LEFT", WD.EncounterNames[v.journalId])
            ...
            if instanceName then
                ...
                WdLib.gui:generateHover(f, hover)
            end
            return f
        elseif index == 3 then
            return WdLib.gui:addNextColumn(WDRM, parent, index, "LEFT", v.role)
        elseif index == 4 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "LEFT", getRuleDescription(v))
            WdLib.gui:generateSpellHover(f, getRuleDescription(v))
            return f
        elseif index == 5 then
            return WdLib.gui:addNextColumn(WDRM, parent, index, "CENTER", v.points)
        elseif index == 6 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "CENTER", WD_BUTTON_EDIT)
            f:EnableMouse(true)
            f:SetScript("OnClick", function() editRuleLine(v); end)
            f.t:SetColorTexture(.2, 1, .2, .5)
            return f
        elseif index == 7 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "CENTER", WD_BUTTON_DELETE)
            f:EnableMouse(true)
            f:SetScript("OnClick", function() table.remove(WD.db.profile.rules, row); updateRuleLines(); end)
            f.t:SetColorTexture(1, .2, .2, .5)
            return f
        elseif index == 8 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "CENTER", WD_BUTTON_EXPORT)
            f:EnableMouse(true)
            f:SetScript("OnClick", function() exportRule(v); end)
            f.t:SetColorTexture(1, .2, .2, .5)
            return f
        elseif index == 9 then
            local f = WdLib.gui:addNextColumn(WDRM, parent, index, "CENTER", WD_BUTTON_SHARE)
            f:EnableMouse(true)
    ...
    WdLib.gui:updateScrollableTable(WDRM, maxHeight, topLeftPosition, rowsN, columnsN, createFn, updateFn)
end
...
--[[
    ...
    WdLib.gui:showHiddenEditBox(r, "arg2_edit", 0)
    arg2_edit.label:SetText("Points:")
end
...
local function initNewRuleWindow()
    ...
    -- roles filter
    r.menus["roles"] = WdLib.gui:createDropDownMenu(r, "ANY", WdLib.gui:convertTypesToItems(WD.RoleTypes))
    ...
    r.menus["roles"].label = WdLib.gui:createFontDefault(r.menus["roles"], "RIGHT", "Role:")
    r.menus["roles"].label:SetSize(x - 5, 20)
    r.menus["roles"].label:SetPoint("TOPLEFT", r.menus["encounters"].label, "BOTTOMLEFT", 0, -1)
    -- events menu
    local items = WdLib.gui:convertTypesToItems(WD.EventTypes, updateNewRuleMenu)
    ...
    r.hiddenMenus["arg0_edit"].label = WdLib.gui:createFontDefault(r.hiddenMenus["arg0_edit"], "RIGHT", "")
    ...
    r.hiddenMenus["arg1_drop"] = WdLib.gui:createDropDownMenu(r, "Select aura action", {{name = "apply"},{name = "remove"}})
    ...
    r.hiddenMenus["arg1_edit"].label = WdLib.gui:createFontDefault(r.hiddenMenus["arg1_edit"], "RIGHT", "")
    ...
    r.hiddenMenus["arg2_edit"].label = WdLib.gui:createFontDefault(r.hiddenMenus["arg2_edit"], "RIGHT", "")
    ...
    r.bg = WdLib.gui:createColorTexture(r, "ARTWORK", 0, 0, 0, 1)
    r.bg:SetAllPoints()
...
local function initNotifyRuleWindow()
    ...
    r.menus["encounters"] = WdLib.gui:createDropDownMenu(r, "Select encounter", WD:CreateTierList(notifyRule))
...
local function initExportEncounterWindow()
    ...
    r.menus["encounters"] = WdLib.gui:createDropDownMenu(r, "Select encounter", WD:CreateTierList(tryExportEncounter))
...
local function initExportWindow()
    ...
    r.editBox = WdLib.gui:createEditBox(r)
...
local function initImportEncounterWindow()
    ...
    r.button = WdLib.gui:createButton(r)
    ...
    r.button.txt = WdLib.gui:createFont(r.button, "CENTER", WD_BUTTON_IMPORT)
...
local function initShareEncounterWindow()
    ...
    r.menus["encounters"] = WdLib.gui:createDropDownMenu(r, "Select encounter", WD:CreateTierList(share))
...
function WDSimpleRulesModule:init(parent, yOffset)
    WD.Module.init(self, WD_BUTTON_ENCOUNTERS_MODULE, parent, yOffset)
    ...
    -- new rule button
    WDRM.buttons["add_rule"] = WdLib.gui:createButton(WDRM)
    ...
    WDRM.buttons["add_rule"].txt = WdLib.gui:createFont(WDRM.buttons["add_rule"], "CENTER", WD_BUTTON_NEW_RULE)
    WDRM.buttons["add_rule"].txt:SetAllPoints()

    -- notify rules button
    WDRM.buttons["notify"] = WdLib.gui:createButton(WDRM)
    ...
    WDRM.buttons["notify"].txt = WdLib.gui:createFont(WDRM.buttons["notify"], "CENTER", WD_BUTTON_NOTIFY_RULES)
    WDRM.buttons["notify"].txt:SetAllPoints()

    -- export encounter button
    WDRM.buttons["export"] = WdLib.gui:createButton(WDRM)
    ...
    WDRM.buttons["export"].txt = WdLib.gui:createFont(WDRM.buttons["export"], "CENTER", WD_BUTTON_EXPORT_ENCOUNTERS)
    WDRM.buttons["export"].txt:SetAllPoints()

    -- import encounter button
    WDRM.buttons["import"] = WdLib.gui:createButton(WDRM)
    ...
    WDRM.buttons["import"].txt = WdLib.gui:createFont(WDRM.buttons["import"], "CENTER", WD_BUTTON_IMPORT_ENCOUNTERS)
    WDRM.buttons["import"].txt:SetAllPoints()

    -- share encounter button
    WDRM.buttons["share"] = WdLib.gui:createButton(WDRM)
    ...
    WDRM.buttons["share"].txt = WdLib.gui:createFont(WDRM.buttons["share"], "CENTER", WD_BUTTON_SHARE_ENCOUNTERS)
    WDRM.buttons["share"].txt:SetAllPoints()

    -- headers
    local x, y = 1, -30
    WDRM.headers = {}
    local h = WdLib.gui:createTableHeader(WDRM, "", x, y, 20, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeader(WDRM, WD_BUTTON_ENCOUNTER, x + 21, y, 200, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, WD_BUTTON_ROLE, 75, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, WD_BUTTON_REASON, 450, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, WD_BUTTON_POINTS_SHORT, 50, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, "", 50, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, "", 50, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, "", 50, 20)
    table.insert(WDRM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDRM, h, "", 70, 20)
    table.insert(WDRM.headers, h)
