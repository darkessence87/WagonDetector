...
local WDHistoryModule = {}
WDHistoryModule.__index = WDHistoryModule

setmetatable(WDHistoryModule, {
    __index = WD.Module,
    __call = function (v, ...)
...
local function exportHistory()
    ...
    for k,v in pairs(history) do
        if not v.isReverted or v.isReverted == false then
            ...
            v.n = WdLib.gen:getShortName(v.name)
            v.name = nil
...
local function refreshHistoryFrame()
    ...
    local function createFn(parent, row, index)
        ...
        if index == 1 then
            local f = WdLib.gui:addNextColumn(WDHM, parent, index, "LEFT", v.timestamp)
            f:SetPoint("TOPLEFT", parent, "TOPLEFT", 0, -1)
            return f
        elseif index == 2 then
            return WdLib.gui:addNextColumn(WDHM, parent, index, "LEFT", v.encounter)
        elseif index == 3 then
            ...
            return WdLib.gui:addNextColumn(WDHM, parent, index, "LEFT", fuckerName)
        elseif index == 4 then
            return WdLib.gui:addNextColumn(WDHM, parent, index, "CENTER", v.role)
        elseif index == 5 then
            return WdLib.gui:addNextColumn(WDHM, parent, index, "CENTER", v.points)
        elseif index == 6 then
            local f = WdLib.gui:addNextColumn(WDHM, parent, index, "LEFT", v.reason)
            WdLib.gui:generateSpellHover(f, v.reason)
            return f
        elseif index == 7 then
            local f = WdLib.gui:addNextColumn(WDHM, parent, index, "CENTER", WD_BUTTON_REVERT)
            f:EnableMouse(true)
            f.t:SetColorTexture(.2, .6, .2, .7)
            f:SetScript("OnClick", function() revertHistory(v); refreshHistoryFrame() end)
            return f
        elseif index == 8 then
            local f = WdLib.gui:addNextColumn(WDHM, parent, index, "CENTER", WD_BUTTON_DELETE)
            f:EnableMouse(true)
    ...
    WdLib.gui:updateScrollableTable(WDHM, maxHeight, topLeftPosition, rowsN, columnsN, createFn, updateFn)
end
...
local function initFiltersTab()
    ...
    --[[WDHM.clearButton = WdLib.gui:createButton(WDHM)
    ...
    WDHM.clearButton.txt = WdLib.gui:createFont(WDHM.clearButton, "CENTER", WD_BUTTON_CLEAR)
    WDHM.clearButton.txt:SetAllPoints()
    WDHM.clearButton.t:SetColorTexture(.6, .2, .2, .7)]]

    WDHM.encounterFilter = WdLib.gui:createEditBox(WDHM)
    ...
    WDHM.nameFilter = WdLib.gui:createEditBox(WDHM)
    ...
    WDHM.roleFilter = WdLib.gui:createEditBox(WDHM)
    ...
    WDHM.pointsFilter = WdLib.gui:createEditBox(WDHM)
    ...
    WDHM.reasonFilter = WdLib.gui:createEditBox(WDHM)
    ...
    WDHM.export = WdLib.gui:createButton(WDHM)
    ...
    WDHM.export.txt = WdLib.gui:createFont(WDHM.export, "CENTER", WD_BUTTON_EXPORT)
...
function WDHistoryModule:init(parent, yOffset)
    WD.Module.init(self, WD_BUTTON_HISTORY_MODULE, parent, yOffset)
    ...
    WDHM.headers = {}
    local h = WdLib.gui:createTableHeader(WDHM, WD_BUTTON_TIME, x, y, 70, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, WD_BUTTON_ENCOUNTER, 200, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, WD_BUTTON_NAME, 100, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, WD_BUTTON_ROLE, 55, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, WD_BUTTON_POINTS_SHORT, 40, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, WD_BUTTON_REASON, 470, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, "", 45, 20)
    table.insert(WDHM.headers, h)
    h = WdLib.gui:createTableHeaderNext(WDHM, h, "", 45, 20)
    table.insert(WDHM.headers, h)
...
WD.HistoryModule = WDHistoryModule
